...
- [ ] **Domain subscribers ignore the new cancel kinds.** `BountyEscrowSubscriber`, `HackathonEscrowSubscriber`, `GrantEscrowSubscriber`, and `CrowdfundingEscrowSubscriber` switch on `EscrowOpKind.CANCEL_EVENT` to flip the domain row to CANCELLED. With the audit fix the cancel flow emits `START_CANCEL` / `PROCESS_CANCEL_BATCH` / `FINALIZE_CANCEL` instead, so the bounty / hackathon / grant / campaign row stays in its prior status even though the on-chain refund happened. Fix: each subscriber should react to `FINALIZE_CANCEL` (or to `START_CANCEL` when the on-chain status is already Cancelled — the OwnerOnly inline path) and mark the domain row CANCELLED. Surface: discovered by `scripts/smoke/bounty-cancel.ts` after the 2026-06-05 testnet redeploy.
- [ ] **Crowdfunding `claim_milestone` orchestrator path needs admin co-sign.** M5 added `admin.require_auth()` for `Pillar::Crowdfunding`. The current `CrowdfundingMilestoneEscrowService` / orchestrator path produces an XDR signed only by the builder, so the tx fails on chain with `Error(Auth, InvalidAction)` for the admin address. Fix: add an admin sign step to the crowdfunding claim path (in both the orchestrator's signing flow and the testnet smoke). Surface: discovered by `scripts/smoke/crowdfunding-publish-and-claim.ts`.
