...
- [ ] **On-chain credit removal: mainnet storage migration.** 2026-06-28: dropped the on-chain credit surface from both contracts (source + tests green, 41 profile / 150 events). Profile struct went `{ bootstrapped_at, credits, reputation }` -> `{ bootstrapped_at, reputation }`; `spend_credits` / `earn_credits` / `refund_credits` / `admin_grant_credits` / `set_default_bootstrap_credits` removed; events contract dropped `application_credit_cost` (EventRecord / CreateEventParams) and `credit_earn` (WinnerSpec). Credits are off-chain only now (see `boundless-credits-reputation-prd.md` §2). **This is a breaking storage-layout change on the LIVE mainnet profile contract.** Existing Profile rows were serialized with a `credits` key; the new struct will fail to deserialize them. Before the upgrade ships, the `migrate()` path MUST rewrite every persisted Profile row stripping `credits` (and needs an enumerable list of bootstrapped user addresses, since the contract does not index them on-chain). Sequence: snapshot/backup -> register the new wasm via the timelocked `propose_upgrade` -> `apply_upgrade` -> `migrate()` -> verify reads. Multisig governance op per `docs/mainnet-deploy-runbook.md` + `docs/admin-custody-policy.md`; NOT yet executed.
