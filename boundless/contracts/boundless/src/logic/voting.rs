use crate::{
    datatypes::{BoundlessError, DataKey, Project, ProjectStatus, VOTING_PERIOD_LEDGERS}, interface::{ProjectManagement, VotingOperations}, BoundlessContract, BoundlessContractArgs, BoundlessContractClient
};
...
impl VotingOperations for BoundlessContract {
    fn vote_project(env: Env, project_id: String, voter: Address, vote_value: i32) -> Result<(), BoundlessError> {
        ...
        if (total_votes as u32) >= voting_threshold && positive_votes > (total_votes / 2).try_into().unwrap() {
            ...
            env.events().publish(
                (DataKey::Project(project_id.clone()), symbol_short!("status")), 
                (project.project_id.clone(), ProjectStatus::Funding)
            );
        }
        
        env.storage().persistent().set(&DataKey::Project(project_id.clone()), &project);
        env.events().publish((DataKey::Project(project_id.clone()), symbol_short!("voted")), project.project_id);        
    ...
    fn withdraw_vote(env: Env, project_id: String, voter: Address) -> Result<(), BoundlessError> {
        ...
        env.storage().persistent().set(&DataKey::Project(project_id.clone()), &project);
        env.events().publish((DataKey::Project(project_id.clone()), symbol_short!("withdrawn")), project.project_id);
        Ok(())
    }
